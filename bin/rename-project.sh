...
main () {
    assert_clean_work_tree
    read -p "Renaming 'movie-database' to: " ns
    search_and_replace "movie-database" "$ns"
    local fdir="${ns//-/_}"
    search_and_replace "movie_database" "$fdir"
    rename_matching_dirs "movie_database" "$fdir"
}
