...
Master: <img src="https://api.travis-ci.org/fulcrologic/movie-database.svg?branch=master"></img>
Develop: <img src="https://api.travis-ci.org/fulcrologic/movie-database.svg?branch=develop"></img>

## Features

The app supports a fake user database (see `valid-users` in `src/main/movie_database/api/mutations.clj`)
...
will build `target/movie_database.jar`.
...
java -Dconfig=config/prod.edn -jar movie_database.jar
```
